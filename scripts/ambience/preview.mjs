...
// can answer. The check script enforces the measurable floor (energy distribution,
// loudness range, spectral separation); this is how a human judges the rest.
//
...
// treat the preview as a faithful audition, not a byte-exact simulation.
...
import {
  ...
  decodeWav, encodeWav, renderIR, convolve, poissonTimes,
  filter, mix, scale, addAt, normalizeRms, rms, softClip, panMono, decorrelate,
  lowEnergyRatio, loudnessRangeDb,
} from './dsp.mjs';
...
//
// Every kind in services/ambiencePresets.ts must appear here or the preview would
// silently omit part of the scene, which would make it untrustworthy for judging.
// ---------------------------------------------------------------------------
const EVENT_RENDERERS = {
  ...
  footstep: (sr, rng, room) => E.footstepRun(sr, {
    surface: surfaceForRoom(room), rng, steps: randInt(rng, 2, 5), tempoHz: rand(rng, 1.7, 2.2),
  }).buffer,
  footstepRun: (sr, rng, room) => E.footstepRun(sr, {
    surface: surfaceForRoom(room), rng, steps: randInt(rng, 5, 10), tempoHz: rand(rng, 1.8, 2.4),
  }).buffer,
...
// ---------------------------------------------------------------------------
// Load the scene recipes out of the TypeScript source.
// ---------------------------------------------------------------------------
...
function loadStemBuffer(stemId) {
  ...
  // Naive linear resample to the preview rate. Fine for auditioning.
  ...
  return channels.map((chan) => {
    ...
    for (let i = 0; i < outLen; i++) {
      ...
      const frac = pos - i0;
      out[i] = chan[i0] * (1 - frac) + (chan[Math.min(chan.length - 1, i0 + 1)] ?? 0) * frac;
    }
    return out;
  });
}

function renderScene(sceneId, recipe, seconds, seed, bedLevel) {
  ...
  for (const layer of recipe.stems) {
    ...
    const sl = shape(l);
    const sr2 = shape(r);
    // Loop the stem across the preview length, entering at a random offset exactly
    // as the engine does.
  ...
  for (const spec of recipe.events ?? []) {
    ...
    for (const t of times) {
      let buf = render(SAMPLE_RATE, rng, recipe.room.size);
      let bl;
      let br;
  ...
  return { channels: [outL, outR], counts, missing };
}
...
for (const sceneId of targets) {
  ...
  const { channels, counts, missing } = renderScene(sceneId, recipe, seconds, seed, bedLevel);
  ...
  console.log(`  mix    <250Hz ${(low * 100).toFixed(0)}%  LRA ${lra.toFixed(1)} dB  (${((Date.now() - started) / 1000).toFixed(1)}s)`);
