...
- **Intermediate (B1-B2)**: Bridging inference, problem↔solution, graded attitude, lexical precision and collocation — **six exercises**. Introduces V/F/NOT GIVEN to penalise over-inference.
- **Advanced (C1)**: Implicature, irony, subtext, rhetorical organisation, hedging, diaphasic/diastratic variation — **six exercises**.

B1-B2 and C1 were 9 and 10 slots (≈32 and ≈38 discrete answers) until the trim: they had never been cut while A0 and A1-A2 had, and much of the excess was repetition — C1 graded `actitud_postura` twice over the same audio (`c1-concesiones` *and* `c1-compromiso`) and asked for the same nuance twice (`c1-matiz`, `c1-registro`). More exercises is not more listening; past a point it is only more reading.
...
- **Exercise budget per level.** `scripts/check-syllabus.mjs` caps lessons at **3 / 4 / 6 / 6** slots (A0 / A1-A2 / B1-B2 / C1), and every mode is at or under its cap. The blueprint *is* the lesson — there is no trimming step downstream — so a slot added anywhere has to displace another one.
