...
- **Exercise budget per level.** `scripts/check-syllabus.mjs` caps lessons at 4 / 6 / 9 / 10 slots (A0 / A1-A2 / B1-B2 / C1). The blueprint *is* the lesson, and `fillMissingSlots()` now enforces that: a slot added to a low level has to displace another one, and **anything the model returns beyond the blueprint is discarded**. That last part used to be false — surplus exercises were appended at the end under the comment "valid exercises the model added extra: kept at the end" — and it silently defeated the budget. The format a model improvises when it overproduces is `multiple_choice`, so a three-slot A0 lesson reached the learner with **eight exercises, six of them unrequested multiple choice**, none of them about the dictated datum the level exists to train. `check:exercises` now pins the ceiling.
...
3. `fillMissingSlots(verified, blueprint, dialogue)` (`services/exerciseEngines.ts`) matches each verified exercise to its slot, fills what's left empty with deterministic engines **in the slot's position**, and drops the surplus. Everything it builds goes through the same verifier. Three rules it enforces, each of which was a real defect:
   - **Slots are claimed by name before they are claimed by format.** Matching was a single pass in blueprint order, so an early slot took by *format* the exercise a later slot had already claimed by `slotId` — with three improvised multiple choices at A0, the first one won `a0-global` even when the right one came labelled.
   - **A slot with `focus` only accepts a model exercise that actually covers the datum** (`coversFocus()`, reusing `isFocusLiteral()`). The verifier only checks that captured values are *said*, and "un café" is said, so a lesson about a dictated phone number could ship a data-capture form asking for the drink, the time and the name. Off-focus exercises fall through to the engine, which reads `slot.focus`; if the engine can't build anything, the model's exercise is kept rather than leaving a hole.
   - **The surplus is discarded, not appended.** See the exercise budget above.
...
- Generated lessons are cached in **IndexedDB** (`services/lessonCache.ts`), keyed by `{SYLLABUS_VERSION, mode, level, textType, accent, length, topic}`, LRU-capped at 20 entries. **`SYLLABUS_VERSION` is part of the key on purpose**: a cached lesson is the *whole* lesson, exercises included, so without it a change to a level's blueprint had no effect for anyone who had already generated that configuration — they kept being served the old lesson, with the old exercises, until they hit «Regenerar» on each one. Bump it whenever a level or mode blueprint changes; entries from an earlier version are deleted on the next write instead of waiting for LRU eviction, since each one holds several MB of PCM. The dialogue is requested with `temperature: 0.0`, so repeating a configuration used to re-pay the whole pipeline for essentially the same lesson. Only *complete* lessons (plan **and** audio) are stored — a cached lesson without audio would have to go back to the TTS anyway. `AppMode.AccentChallenge` is **never** cached: it draws two random accents per run, so its key does not describe its contents. A «Regenerar» button in the header invalidates the entry and forces a fresh lesson. The PCM is stored as raw bytes and re-encoded to base64 on read, so the app boundary is unchanged.
